...
try {
  if ($action === 'list') {
    ...
    $sql .= " ORDER BY p.destacado DESC, p.idplato, pxi.ididioma";
  ...
  if ($action === 'create') {
    ...
    $pdo->prepare("INSERT INTO platos (idcategoria, precio, image, destacado) VALUES (?,?,?,?)")
        ->execute([$idcategoria, $precio, $image, $destacado]);
